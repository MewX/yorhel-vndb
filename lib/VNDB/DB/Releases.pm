...
sub dbReleaseGet {
  ...
  my @select = (
    qw|r.id r.title r.original r.website r.l_steam r.l_dlsite r.l_dlsiteen r.released r.minage r.type r.patch|,
...
sub dbReleaseGetRev {
  ...
  my $select = 'c.itemid AS id, r.title, r.original, r.website, r.l_steam, r.l_dlsite, r.l_dlsiteen, r.released, r.minage, r.type, r.patch';
...
sub dbReleaseRevisionInsert {
  ...
  my %set = map exists($o->{$_}) ? ("$_ = ?", $o->{$_}) : (),
    qw|title original gtin catalog website l_steam l_dlsite l_dlsiteen released notes minage type
